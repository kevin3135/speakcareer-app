...
export function ProgressScreen({ dailyTarget, onOpenRoleplay, sessions }: ProgressScreenProps) {
  ...
  const nextRoleplay = practiceContent.roleplays.find((roleplay) => roleplay.id === 'meeting-practice') ?? practiceContent.roleplays[0];
  ...
  return (
    ...
    >
      ...
      <Card tone="strong">
        <View style={styles.rowBetween}>
          <View style={styles.flexOne}>
            <Text style={styles.cardKicker}>Daily goal</Text>
            <Text style={styles.cardTitle}>
              {localProgress.targetSessionsCompleted}/{dailyTarget} roleplays today
            </Text>
          </View>
          <XPBadge label={mission.rewardLabel} />
        </View>
        <View style={styles.progressWrap}>
          <ProgressBar label="Goal progress" value={localProgress.targetCompletionPercent} tone="secondary" />
        </View>
        <View style={styles.cardAction}>
          <AppButton
            label="Start next roleplay"
            onPress={() => onOpenRoleplay(nextRoleplay.id)}
          />
        </View>
      </Card>
      ...
      {latestSession ? (
        ...
        <Card tone="muted">
          <Text style={styles.cardKicker}>Latest win</Text>
          <Text style={styles.cardTitle}>Save your first answer</Text>
          <Text style={styles.sessionFeedback}>Your first saved roleplay will appear here with XP and feedback.</Text>
        </Card>
      )}
